...
public class ComputerPlayer extends Player {
	...
	public String determineMove(Board board) {
		...
		while (elapsedTime < 0.9 * Protocol.Settings.TIMEOUTSECONDS * 1000) {
			...
			if (maxIndex < 6 && max > 0) { // 0 t/m 5 betekent dezelfde kleur, 6 t/m 11 betekent dezelfde vorm
				...
				for (Coordinate c : possibleMoves) {
					String s = checkCoordinate(board.deepcopy(),c,tileGrouping.get(maxIndex),color,shapes);
					if (!s.equals(ClientHandler.NOREPLY) &&
							Integer.parseInt(s.split(Character.toString(Protocol.Settings.DELIMITER))[0]) > bestScore) {
						bestScore = Integer.parseInt(s.split(Character.toString(Protocol.Settings.DELIMITER))[0]);
						bestMove = s;
					}
				}
			}
    ...
    private boolean movePossible(Tile[][] fields, int i, int j) {
		return (fields[i][j+1] != null || fields[i][j-1] != null || fields[i-1][j] != null || fields[i+1][j] != null);		
	}
				...
				if (!shapes.contains(s)) {
					...
					if (index >= 0) {
						return Protocol.Client.MAKEMOVE + Protocol.Settings.DELIMITER + 
								getHand()[index].getColor().getCharColor() + getHand()[index].getShape().getCharShape() + 
								Protocol.Settings.DELIMITER2 + xValue + Protocol.Settings.DELIMITER2 + yValue;
					}
				}
			}
    ...
    public String checkCoordinate(Board b, Coordinate c, ArrayList<Tile> tiles, Color color, ArrayList<Tile.Shape> shapes) {
    	/* deze checkt alleen voor kleur en alleen oostelijk
    	 * Zuid,Noord, West moet ook nog in deze methode
    	 * Voor shape doen we in een andere methode
    	 */
    	...
    	if (bestMove[0][0] != -1) {
    		...
    		while (bestMove[i][0] != -1) {
    			msg += getHand()[bestMove[i][2]].getColor().getCharColor() + getHand()[bestMove[i][2]].getShape().getCharShape() +
					Protocol.Settings.DELIMITER2 + bestMove[i][0] + Protocol.Settings.DELIMITER2 + bestMove[i][1] + Protocol.Settings.DELIMITER;
    		i++;
    		}
